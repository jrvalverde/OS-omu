...
printf("INternal exec return\n\r");
	...
	case 65:
		...
		REG(d0) = comms(REG(a0), REG(d1), REG(a1));
		break;
	...
	case 83:
		/* Pre-Wakeup on this event */
		REG(d0) = wakeon((caddr_t)REG(a0));
		break;
	...
	case 85:
		/* Set Signal mode */
		REG(d0) = sigmode(REG(a0));
		break;

	case 86:
		/* Mask signals */
		REG(d0) = sigmask(REG(a0));
		break;

	case 87:
		/* Dup file channel across processes */
		REG(d0) = pdup(REG(a0), REG(d1), REG(a1));
		break;

	case 88:
		/* Set trap to point to device driver */
		REG(d0) = settrap(REG(a0), (caddr_t)REG(d1));
		break;

	case 89:
		/* Clear trap to device driver */
		REG(d0) = clrtrap(REG(a0), (caddr_t)REG(d1));
		break;
...
#ifdef	VIWINDOWS
	case 91:
		/* Initialise window function call hacks uhhh! */
		REG(d0) = initwin(REG(a0), REG(d1), REG(a1));
		break;
#endif
	case 92:
		/* Find out if file channel is a pipe */
		REG(d0) = ispipe(REG(a0));
		break;
