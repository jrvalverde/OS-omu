 ...
 */
	...
	/* Checks if any other process wants to run */
	checkpri();
...
/*
 *	Checkpri()	Checks process prioritys, if any want to run
 *			at system priority level indicate that a process
 *			switch should occur;
 */
checkpri(){
	register struct	procs *proc;
	...
	proc = &proc_table[0];
	for(pid = 0; pid < NPROC; pid++){
		if((proc->stat == SRUN) && (proc->pri < PUSERMIN)){
			swtchflag = 1;
			break;
		}
		proc++;
