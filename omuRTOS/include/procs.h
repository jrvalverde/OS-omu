...
struct u_regs {
	...
	short		vec;		/* Exception vector (68010 only) */
};
...
struct procs {
	...
	short		sigmode;	/* Signal mode */
	unsigned long	sig;		/* Signals pending bit map */
	unsigned long	sigmask;	/* Signals Mask bit map */
	struct signal	signals[NSIG];	/* Signals */
	unsigned long	evenfile;	/* File event enable bit map */
	unsigned long	evfile;		/* File event pending bit map */
	short		evcom;		/* Comms event pending */
	long		evcoma[3];	/* Comms event arguments */

	caddr_t		pwakechan;	/* Pre-wake value */
	short		pwakeevent;	/* Pre-wake event occured */
	...
	short		stackover;	/* Stack overflow test location */
...
/*	Prewake	This entry if pwakechan is set to a valid event pre-empting
 *		of the event sleep system will occur.
 *		ie If this event occurs the pwakeevent entry will be set
 *		to indicate that the prewake has occured
 *		if the process go's to sleep on this event it will be
 *		immediatly awoken.
 */
# define	PWAKEOFF	0	/* No prewake */
# define	PWAKEEVENT	1	/* Prewake event occured */
