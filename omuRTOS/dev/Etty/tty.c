...
#ifdef	VIWINDOWS
/*
 *	Setpoint()	Horrible routine !, This calls the kernal function
 *			winset(), which has been set by the window manager
 *			To point to one of its internal routines.
 *			If a point is found, and it is not the first to the
 *			Window, then return 1, else return 0;
 */
setpoint(tty)
struct ttystruct *tty;
{
	/* Check if point is valid, if not return 0
	 * NOTE: the window manager auto-maticaly activates the touched window
	 * And brings it forward, if in TTYWINACT mode.
	 */
	if(winset(&tty->wintty.ipoint, (tty->wintty.wmode & TTYWINACT) ? 1 : 0)) return 0;
	else {
		/* If TTYWINAUTO mode and If new process owns touched display,
		 * flush buffer, set new process id but return 0, so that this
		 * point is not sent to the new task.
		 */
		if(tty->wintty.wmode & TTYWINAUTO){
			if(tty->wintty.cpid != tty->wintty.ipoint.pid){
				tty->wintty.cpid = tty->wintty.ipoint.pid;
				flush(&tty->inbuf);
				if(tty->wintty.wmode & TTYWINIM) return 1;
				else return 0;
			}
			else return 1;
		}
 ...
 *		in polled mode
 *		If characters in expansion buffer gets those else gets
 *		from normal buffer.
 */
ttyout(dev)
short dev;
{
	...
	/* Checks if any expansion characters if so output these */
	if(tty->nexpand){
		tty->nexpand--;
		writechar(dev, *(tty->exp++));
		return 1;
	}
	/* Else trys to get character from buffer if none sets tx_rdy flag */
	else if(!tgetbuf(&tty->outbuf,&ch,1)){
		tty->tx_rdy = RDY;

		/* Check if buffer is valid if so wake up any processes wait */
	...
	/* If not raw do any output conversions */
		...
		case TAB:
			...
			if((tty->sgtty.sg_flags & XTABS) && nchars--){
				/* Add number of tab spaces - 1 to expand buf */
				for (cnt = 0; cnt < nchars; cnt++){
					tty->expand[cnt] = ' ';
				}
				tty->exp = tty->expand;
				tty->nexpand = nchars;
				writechar(dev, ' ');	/* First space */
			}
		...
		case NL:
			...
			if(tty->sgtty.sg_flags&CRMOD){
				...
				/* Add nl character to expand buffer */
				tty->expand[0] = NL;
				tty->exp = tty->expand;
				tty->nexpand = 1;
				tty->line++;
...
/*
 *	Writechar	Will wait until a character is ready
 *			passing through ttywait(), to indicate
 *			that the tty handler is dossing around again.
...
short number;
{
	...
	if(tbuf->count < number) number = tbuf->count;
	num = number;
	topbuf = &tbuf->buf[TTYBUFSIZE];	/* Address of buffer top */
...
int *argp;
{
	...
	case TIOCGWINDOW:
		/* Window mode */
		*argp = tty->wintty.wmode;
		break;

	case TIOCSWINDOW:
		/* Flush input buffer of tty */
		flush(&tty->inbuf);

		/* Set window mode */
		tty->wintty.wmode = *argp;

		/* If one tty mode, set the TTY handler to point to this
		 * task only
		 */
		if(tty->wintty.wmode & TTYWINON){
			if(!(tty->wintty.wmode & TTYWINAUTO)){
	...
	case TIOCGPIDWIN:	/* Returns current pid of terminal */
		*argp = tty->wintty.cpid;
		break;

	case TIOCSPIDWIN:	/* Sets current pid of terminal */
		tty->wintty.cpid = *argp;
		break;
#endif
