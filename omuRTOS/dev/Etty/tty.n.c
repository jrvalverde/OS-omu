...
o_tty(dev)
{
	...
	if (dev < NDEVS){
		...
		if (tty->nopens++ == 0){
			...
			tty->tx_ewait = 0;	/* Normal output of buffer */
...
char type;
{
	...
	case WWAITB:
		...
		break;
	case WWAITC:
		/* Sets process to sleep on txbuf waiting for character to
		 * be written.
		 */
		ttys[dev].tx_ewait = 1;
		sleep((caddr_t)&ttys[dev].outbuf,PTTY);
...
short dev;
{
	...
	/* Checks if waiting for an extended character to be output
	 * eg CRNL, tabs etc. if so return waking up process
	 */
	if(tty->tx_ewait){
		wakeup((caddr_t)&tty->outbuf);
		return 0;
	}
